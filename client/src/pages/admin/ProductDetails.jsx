...
export default function ProductDetails() {
  ...
  return (
    <main className="m-5 flex flex-1 flex-col gap-2 text-white">
      ...
      <span className="text-center text-2xl font-semibold">Product Details of {!error && !loading && data.data[0].ProductName}</span>
        ...
        <div className="flex flex-wrap justify-evenly gap-5 py-2 text-black">
          ...
          {!loading &&
            ...
            data.data[0]?.ProductVariants?.map((product) => (
              <div key={product.ProductSizeID} className="card w-[10rem] bg-white shadow-xl hover:ring-2 hover:ring-black lg:w-[17rem]">
                <div className="card-body px-0.5 text-center lg:px-2">
                  <div className="card-title text-start">
                    <h2 className="flex-1 text-start text-xs font-semibold lg:text-lg">{product.ProductSizeName}</h2>
                  </div>
                  <span className="text-start text-xs font-bold text-black lg:text-base">{formatCurrency(product.ProductPrice)}</span>

                  {product.ProductVariantName !== "NULL" && <span className="text-start">{product.ProductVariantName}</span>}

                  <div className="card-actions mt-1 flex justify-center text-xs">
                    <Button className="btn btn-primary flex-1">Add To Cart</Button>
                    <Button className="btn btn-primary flex-1">Process Order</Button>
                  </div>
                </div>
              </div>
            ))}
